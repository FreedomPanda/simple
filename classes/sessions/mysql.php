...
class Mysql extends Session
{
	...
	{
		return ORM::factory($this->_table)->where([$this->_columns['session_id']	=> $this->_session_id])->save([
	...
	protected function _regenerate()
	{
		do
		{
			...
			$id = self::_make_id();
			$data = ORM::factory($this->_table)->where(array($this->_columns['session_id'] => $id))->find();
		}
		while ($data);

		$this->_session_id = $id;
		$this->_data = [];
		ORM::factory($this->_table)->add([
	...
	{
		//删除最后使用日期在上个月以前的所有条目
		ORM::factory($this->_table)->where([$this->_columns['last_active'] => array('<', time()-2629744)])->delete();
	}
}